...
<section class="section">
    <div class="container-fluid">
        ...
        <div class="card col-md-6">
            <div class="card-body">
                <form action="?page=produk_ubah_proses" method="post" autocomplete="off" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="<?= $pro['id'] ?>">
                    <div class="input-style-1">
                        <label for="nama">Nama Produk</label>
                        <input type="text" value="<?= $pro['nama'] ?>" class="form-control" name="nama" placeholder="Nama Produk" id="nama" required>
                    </div>
                    <div class="input-style-1">
                        <label for="deskripsi">Deskripsi</label>
                        <textarea name="deskripsi" rows="5" id="deskripsi" placeholder="Masukkan Deskripsi Produk" class="form-control"><?= $pro['deskripsi'] ?></textarea>
                    </div>
                    <div class="input-style-1">
                        <label for="harga">Harga Produk</label>
                        <input type="number" value="<?= $pro['harga'] ?>" class="form-control" name="harga" placeholder="Harga Produk" id="harga" required>
                    </div>
                    <div class="input-style-1">
                        <label>Fitur-fitur</label>
                        <div id="fitur-container">
                            <?php foreach ($fitur_array as $index => $fitur): ?>
                                <?php if (!empty($fitur)): ?>
                                    <div class="input-group mb-2">
                                        <input type="text" class="form-control" name="fitur[]" placeholder="Masukkan fitur produk" value="<?= $fitur ?>">
